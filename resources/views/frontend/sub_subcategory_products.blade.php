                                ...
                                <div class="dlab-accordion advanced-search toggle" id="accordion1">
                                        ...
                                        <div class="acod-head">
                                            <h5 class="acod-title">
                                                <a data-toggle="collapse" href="#categories">
                                                      الفئات الفرعية من     {{ \App\Models\Admin\SubSubCategory::find($id)->subcategory->subcategory_name }}
                                                </a>
                                            </h5>
                                        </div>
                                        <div id="categories" class="acod-body collapse show">
                                            <div class="acod-content">
                                                <div class="widget widget_services">
                                                    <ul>
                                                        <?php
                                                            $subcategory_id = \App\Models\Admin\SubSubCategory::find($id)->subcategory->id;
                                                            $subsubcategories = \App\Models\Admin\SubSubCategory::where('subcategory_id',$subcategory_id)->get();
                                                        ?>
                                                        @if(count($subsubcategories) > 0)
                                                            @foreach($subsubcategories as $row)
                                                                    <li><a href="{{ route('products.in.sub_subcategory',[$row->id,$row->subsubcategory_name]) }}">{{ $row->subsubcategory_name }}</a></li>
                                                            @endforeach
                                                        @endif
                                                    </ul>
                                                </div>
                                        ...
                                        <div id="vendor" class="acod-body collapse show">
                                            <div class="acod-content">
                                                <div class="product-brand">
                                                    ...
                                                    <form action="{{ route('price.filter.sub_subcategory',[$id,$nameOfFather]) }}" method="post" id="priceList">
                                                        @csrf
                                    ...
                                    <div class="panel">
                                        ...
                                        <div id="vendor" class="acod-body collapse show">
                                            <div class="acod-content">
                                                <div class="product-brand">
                                                    <form action="{{ route('brand.filter.sub_subcategory',[$id,$nameOfFather]) }}" method="post" id="brandList">
                                                    @csrf
                                    ...
                                    <div class="panel">
                                        <?php
                                            $all_products = \App\Models\Admin\Product::where('subsubcategory_id',$id)->where('status',1)->
                                                where('approved',1)->get();
                                        $subsubcategory_tags = array();
                                        foreach ($all_products as $row ){
                                            foreach( explode(',',$row->product_tags) as $tags){
                                                if(!in_array($tags,$subsubcategory_tags)){
                                                    $subsubcategory_tags[] = $tags;
                                                }
                                        ...
                                        <div id="tags" class="acod-body collapse">
                                            <div class="acod-content">
                                                <div class="widget_tag_cloud radius">
                                                       @foreach($subsubcategory_tags as $tag)
                                                       <a href="{{route('tag.sub_subcategory.products.filter',[$id,$nameOfFather,$tag])}}">{{$tag}}</a>
                                                       @endforeach
