...
<div style="width:100%; background:#e9e9e9; padding:15px 0; direction: rtl; font-size: 16px;">
    ...
    <div style="width:70%; color: #333; background:#fff; margin:20px auto; padding:20px; border:4px solid #BB2B6F; border-radius: 10px; font-family: sans-serif, arial;">
        ...
        أهلا بك, <strong>{{$message_data->name}}</strong>
        <br><br>

        <h2>   استفسارك :
            <br>
            {{ $message_data->message }}  </h2>
        <br>

        <p>  الرد على أستفسارك :
            {{$reply_data->message}} </p>
        <br><br>
    ...
    <div style="margin-top: 20px; text-align: center; font-family: sans-serif; color: #999; font-size: 13px;">
        <div style="margin:0 auto; width:70%;">
            ...
            تم إرسال هذه الرسالة إلى البريد الإلكتروني <a href="mailto:{{$message_data->email}}" style="text-decoration: none; font-weight: bold; color:#487093">{{$message_data->email}}</a>.
            <br>
            إذا اردت عدم استقبال رسائل أخرى فى المستقبل من الموقع من فضلك اضغط
            <a href="{{url('/delete/subscriber/'.$message_data->email)}}" style="text-decoration: none; font-weight: bold; color:#487093" target="new">إلغاء خدمة الرسائل</a>.
